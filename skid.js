...
if (process.argv.length <= 4) {
  console.log("Usage: node skid.js <url> <time> <proxies.txt>"); 
  process.exit(-1);
}
...
let proxies = fs.readFileSync(proxiesFilePath)
  .toString()
  .replace(/\r\n/g, "\n")
  .split("\n")
  .filter(Boolean);
...
if (proxies.length === 0) {
  console.log("No proxies found in proxies.txt file");
  process.exit(-1);
}
...
setInterval(async () => {
  ...
  try {
    ...
    console.log(`Request sent to ${target} with proxy ${proxy}`);
  } catch (error) {
    console.error(`Error while sending request with proxy ${proxy}:`, error.message);
  }
...
setInterval(async () => {
  // logic gửi request sử dụng getRandomUA()
}, 150);
