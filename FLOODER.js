...
let encrypted = cipher.update(randomString, 'utf8', 'hex') + cipher.final('hex');
...
if (!target || !time || !thread || !proxyFile || !rps) {
  console.log('JS-FLOODER'.bgRed)
    console.error(`Example: node ${process.argv[1]} <url> <time> <thread> <proxies file> <rate>`.rainbow);
  process.exit(1);
}

// Validate target format
if (!/^https?:\/\//i.test(target)) {
  console.error('Oops!! you were missing http:// or https://');
  process.exit(1);
}
